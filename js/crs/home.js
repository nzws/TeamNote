...
home.list = function (mode, max_id = 0, load_mode_button) {
  ...
  .then(function(json) {
    if (json["error"]) {
      ...
      if (json[0]) {
        while (json[i]) {
          if (json[i]["edited_by"] && json[i]["edited_by"]["id"] !== json[i]["created_by"]["id"]) json[i]["edited_by_others"] = true;
          reshtml += tmpl(json[i]);
          i++;
        }
        reshtml += `<button class="btn btn-outline-primary btn-block" onclick="home.list('${mode}', ${json[0]["id"]}, this)">もっと見る</button>`;
      }
...
home.search = function (page = 0, load_mode_button) {
  ...
  fetch(apiUrl('note/search') + "?page=" + page + "&word=" + encodeURIComponent(elemId("search_text").value), {
    headers: {'content-type': 'application/x-www-form-urlencoded'},
    method: 'GET',
    credentials: 'include',
  })
